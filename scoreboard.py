...
class Scoreboard:
  ...
  def __init__(self, ai_game):
    ...
    self.font = pygame.font.Font("font/SimHei.ttf", 24)
    self.score_text = '得分'
    self.high_score_text = '最高得分'
    self.level_text = '等级'
    # 准备包含最高分和当前得分的图像
    self.prep_score()
  ...
  def prep_score(self):
    ...
    score_str = f"{self.score_text}:{rounded_score:,}"
  ...
  def prep_high_score(self):
    ...
    high_score_str = f"{self.high_score_text}:{high_score:,}"
  ...
  def prep_level(self):
    """将等级渲染为图像"""
    level_str = f"{self.level_text}:{self.stats.level}"
